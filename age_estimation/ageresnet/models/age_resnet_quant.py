...
import torch
import numpy as np
from quant_layers import new_conv, self_conv, Q_A
...
class BasicBlockQuant(nn.Module):
    ...
    def __init__(self, num_bases, inplanes, planes, bitW, bitA, stride=1, downsample=None, add_gate=True):
        ...
        if add_gate:
            self.block_gate = nn.Parameter(torch.rand(1), requires_grad=True)
...
class ResNetQuant(nn.Module):
    ...
    def __init__(self, block, layers, bitW=32, bitA=32, num_classes=3,num_bases=1,add_gate=False):
        self.inplanes = 64
        self.num_bases = num_bases
        self.add_gate = add_gate
        self.bitW = bitW
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, add_gate= add_gate)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, add_gate= add_gate)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, add_gate= add_gate)  #don't quantize the last layer
    ...
    def forward(self, x):
        ...
        out = self.fc_angles(out)

        return out


def resnet18quant(bitW, bitA, pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetQuant(BasicBlockQuant, [2, 2, 2, 2], bitW, bitA, **kwargs)
    if pretrained:
        load_dict = torch.load('./full_precision_records/model_best.pth.tar')['state_dict']
...
def resnet34quant(bitW, bitA, pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetQuant(BasicBlockQuant, [3, 4, 6, 3], bitW, bitA, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


# def resnet50quant(bitW, bitA, pretrained=False, **kwargs):
#     """Constructs a ResNet-50 model.
#     Args:
#         pretrained (bool): If True, returns a model pre-trained on ImageNet
#     """
#     model = ResNetQuant(Bottleneck, [3, 4, 6, 3], bitW, bitA, **kwargs)
#     return model