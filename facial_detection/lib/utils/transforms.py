...
def transform_pixel(pt, center, scale, output_size, invert=0, rot=0):
    ...
    #np.rint(a)
    #return np.rint(new_pt[:2]).astype(int) + 1
    return (new_pt[:2]).astype(int) + 1

def transform_pixel_pred(pt, center, scale, output_size, invert=0, rot=0):
    # Transform pixel location to different reference
    t = get_transform(center, scale, output_size, rot=rot)
    if invert:
        t = np.linalg.inv(t)
    ...
    #a = np.array([-1.7, -1.5, -0.2, 0.2, 1.5, 1.7, 2.0])
    #np.rint(a)
    #print("new_pt",new_pt[:2])
    #print("new_pt rint",np.rint(new_pt[:2]))
    #print("new_pt int",new_pt[:2].astype(int))
    #return np.rint(new_pt[:2]) #+ 1
    return (new_pt[:2]).astype(int) + 1

def transform_preds(coords, center, scale, output_size):

    for p in range(coords.size(0)):
        coords[p, 0:2] = torch.tensor(transform_pixel_pred(coords[p, 0:2], center, scale, output_size, 1, 0))
    return coords
