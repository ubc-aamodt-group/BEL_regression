...
def convert(encode,num_bits,di,arr):
    encode = torch.tanh(encode)
    t = torch.matmul(encode,di)
    s = nn.Softmax(dim=3)
    t = s(t)
...
def train(config, train_loader, model, criterion, optimizer,
    ...
    for i, (inp, target, meta) in tqdm(enumerate(train_loader)):
        ...
        #outputnew = torch.reshape(output,(output.size(0),int(output.size(1)/68),2,int(68/2)))
        #targetnew = torch.reshape(target,(output.size(0),int(output.size(1)/68),2,int(68/2)))


        #t = (convert_new_tanh_loss(outputnew,int(config.BITS/2),di))
        #targets = torch.clamp(torch.round(meta['tpts']),min=0,max=255).cuda()
        #loss+= 1.0*crit(t.permute(0,3,1,2),torch.reshape(targets.long(),(t.size(0),t.size(1),t.size(2))).permute(0,1,2))
...
def validate(config, val_loader, model, criterion, epoch, writer_dict):
    ...
    with torch.no_grad():
        for i, (inp, target, meta) in enumerate(val_loader):
            ...
            elif config.CODE.CODE_NAME=="BELE1JMJ":
                 preds = convert_be1jmj(output,int(config.CODE.CODE_BITS))
            else:
                 preds = convert(output,int(config.BITS/2),di,arr)
            preds=decode_preds(preds, meta['center'], meta['scale'], [config.BITS, config.BITS])
            ...
            preds=decode_preds(preds, meta['center'], meta['scale'], [config.BITS, config.BITS])
            nme_temp_new = compute_nme(preds.detach(), meta)

            _,_,preds = convert_new_tanh(output,int(config.BITS/2),di)
            preds=decode_preds(preds, meta['center'], meta['scale'], [config.BITS, config.BITS])
            nme_temp_new_tanh = compute_nme(preds.detach(), meta)
            ##preds=preds.data.cpu()
            ##preds=decode_preds(score_map, meta['center'], meta['scale'], [64, 64])
            ...
            failure_008 = (nme_temp_new_tanh > 0.08).sum()
            failure_010 = (nme_temp_new_tanh > 0.10).sum()
            count_failure_008 += failure_008
            count_failure_010 += failure_010

            nme_batch_sum += np.sum(nme_temp)
            nme_batch_sum_new += np.sum(nme_temp_new)
            nme_batch_sum_new_tanh += np.sum(nme_temp_new_tanh)
            nme_count = nme_count + preds.size(0)
    ...
    nme = nme_batch_sum / nme_count
    nme_new = nme_batch_sum_new / nme_count
    nme_new_tanh = nme_batch_sum_new_tanh / nme_count
    ...
    logger.info(msg)
    msg_new = 'Test1 Epoch {} time:{:.4f} loss:{:.4f} nme:{:.4f} [008]:{:.4f} ' \
          '[010]:{:.4f}'.format(epoch, batch_time.avg, losses.avg, nme_new,
                                failure_008_rate, failure_010_rate)
    logger.info(msg_new)
    msg_new_tanh = 'Test2 Epoch {} time:{:.4f} loss:{:.4f} nme:{:.4f} [008]:{:.4f} ' \
          '[010]:{:.4f}'.format(epoch, batch_time.avg, losses.avg, nme_new_tanh,
                                failure_008_rate, failure_010_rate)
