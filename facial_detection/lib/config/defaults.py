...
import os
_C = CN()
_C.OUTPUTNAME= 'output_try'
if (os.environ['TMPDIR']==""):
    os.environ['TMPDIR']="."
_C.OUTPUT_DIR = (os.environ['TMPDIR'])+'/output'
_C.LOG_DIR = 'log'
_C.GPUS = (0, 1, 2, 4)
_C.WORKERS = 16
_C.PRINT_FREQ = 20
...
# code related params
_C.CODE = CN()
_C.CODE.CODE_PKL = "j_256_code.pkl"
_C.CODE.CODE_TENSOR = "j_256_tensor.pkl"
_C.CODE.FC_EXIST = True
_C.CODE.CODE_BITS = 128
_C.CODE.CODE_NAME = "BELJ"
...
_C.TRAIN.LR = 0.0001
_C.TRAIN.PRUNE_STEP = [10, 20,30]
_C.TRAIN.PRUNE_LR_STEP = [20]
_C.PRUNE = 0.0
...
def update_config_code(cfg, args):
    cfg.defrost()
    cfg.merge_from_file(args.code)
    cfg.freeze()
