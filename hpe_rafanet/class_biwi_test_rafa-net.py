...
from ce_custom_validate_callback import CustomCallback
import pickle
from keras.applications.resnet50 import preprocess_input as res50_pp_input
from ce_pose_data_augmentor import DirectoryDataGenerator
from ce_custom_validate_callback import CustomCallback
from ce_custom_validate_callback import validateRegression
...
def ce_loss(y_true, y_pred):
  y_true = tf.reshape(y_true, [16, 360])
  di = pickle.load(open("codes/belj_360_tensor.pkl","rb"))
  di = np.transpose(di).astype('float32')
  t = tf.tensordot(y_pred, di,axes = [[1], [0]])
...
if __name__ == '__main__':
    ...
    print(theta,code_pkl,code_tensor,code_bits,code_name,gamma)
    train_image = 'biwi_rect_70_s3.txt'	
    train_label = 'biwi_angle_70_s3.txt'
    test_image = 'biwi_rect_30_s3.txt'
    test_label = 'biwi_angle_30_s3.txt'
    
    test_files, test_labels, test_Bbox = load_image_files_and_labels(test_image, test_label,radian=False)
    ...
    xb = layers.Dense(code_bits, activation='linear', name="Yaw_outputb")(x)
    #xb = tf.tensordot(xb1, din,axes = [[1], [0]])
    #xb = layers.Dense(360,bias=False, trainable=False,kernel_initializer=tf.keras.initializers.Constant(din),name="Yaw_outputb")(xb1)
    #xb = tf.keras.layers.Dot(axes=(1, 0))([xb1, din])
    ...
    yb = layers.Dense(code_bits, activation='linear', name="Pitch_outputb")(y)
    #yb = tf.tensordot(yb1, din,axes = [[1], [0]])
    #yb = layers.Dense(360,bias=False, trainable=False,kernel_initializer=tf.keras.initializers.Constant(din),name="Pitch_outputb")(yb1)
    #yb = tf.keras.layers.Dot(axes=(1, 0))([yb1, din])
    ...
    zb = layers.Dense(code_bits, activation='linear', name="Roll_outputb")(z)
    #zb = layers.Dense(360,bias=False, trainable=False,kernel_initializer=tf.keras.initializers.Constant(din),name="Roll_outputb")(zb1)
    #zb = tf.tensordot(zb1, din,axes = [[1], [0]])
    #zb = tf.keras.layers.Dot(axes=(1, 0))([zb1, din])
    ...
    losses = {
        ...
        #"Yaw_outputb": tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),# "mean_squared_error",
    	#"Pitch_outputb": tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),# "mean_squared_error",
        #"Roll_outputb": tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)# "mean_squared_error"
        "Yaw_outputb": sparse_loss,# "mean_squared_error",
    	"Pitch_outputb": sparse_loss,# "mean_squared_error",
        "Roll_outputb":sparse_loss # "mean_squared_error"
