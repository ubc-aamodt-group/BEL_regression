...
class DirectoryDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes):
        ...
        Rollb = np.empty((self.batch_size, self.code_bits), dtype=K.floatx())#Yaw, Pitch Roll
        Pitchb = np.empty((self.batch_size, self.code_bits), dtype=K.floatx())
        Yawb = np.empty((self.batch_size, self.code_bits), dtype=K.floatx())
        ...
        for i in range(len(indexes)):
            ...
            Pitch[i]= labels[1]
            Yaw[i]= labels[2]
            Rollb[i] = self.codes[int(round((labels[0]*180/math.pi)+ 180))]
            Pitchb[i] = self.codes[int(round((labels[1]*180/math.pi)+ 180))]
            Yawb[i] = self.codes[int(round((labels[2]*180/math.pi)+ 180))]
