...
def temp_code(num,num_bits):
    a= torch.zeros([num_bits],dtype=torch.long)
    for i in range(0,min(int(num),num_bits)):
        a[num_bits-i]=1
    #a[num_bits]=1
    return a
...
class BIWI_bcd(Dataset):
    def __init__(self, data_dir, filename_path, transform, img_ext='.png', annot_ext='.txt', image_mode='RGB',num_bits=6,code_bits=100,code='u'):
        self.data_dir = data_dir
        self.transform = transform
        self.img_ext = img_ext
        self.annot_ext = annot_ext
        self.code_bits= {'u':[150,100],'j':[75,50],'b1jdj':[39,26],'b2jdj':[21,15],'hex16':[16,16]}
        filename_list = get_list_from_filenames(filename_path)
        ...
        self.code_100=pickle.load(open("code_pkl/bel"+code+"_100_code.pkl","rb"))                
        self.code_150=pickle.load(open("code_pkl/bel"+code+"_150_code.pkl","rb"))                
    ...
    def __getitem__(self, index):
        ...
        yaw = yaw + 74
        pitch =pitch+ 65
        broll = self.code_100[max(min(int(round(roll)),99),0)]
        byaw = self.code_150[max(min(int(round(yaw)),149),0)]
        bpitch = self.code_150[max(min(int(round(pitch)),149),0)]
        biroll = max(min(int(round(roll)),99),0)
        biyaw = max(min(int(round(yaw)),149),0)
        bipitch = max(min(int(round(pitch)),149),0)
