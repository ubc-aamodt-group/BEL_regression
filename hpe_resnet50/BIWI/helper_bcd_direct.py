...
def convert_cor(encode,num_bits,di): # based on correlation
    ...
    return ts
def convert_direct(encode,num_bits,di): # based on correlation
    #t = torch.matmul(encode,di)
    return torch.flatten(encode,start_dim=0)
...
def measure_MAE(cont_labels,yaw,pitch,roll,gpu,func,di,dis):
    ...
    yaw_predicted=func(yaw,150,di).cpu()
    pitch_predicted= func(pitch,150,di).cpu()
    roll_predicted=func(roll,100,dis).cpu()
...
def train(model,train_loader,test_loader, pose_dataset, output_string, gpu, arch,lr,num_epochs,batch_size,val_bound,num_bits,code_bits,code,loss_func,di,dis):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels, cont_labels, name, tyaw,tpitch,troll,tiyaw,tipitch,tiroll) in tqdm((enumerate(train_loader))):
            ...
            #print(cont_labels[0],yaw[0],tyaw[0])
            ...
            if loss_func=="bce":
                ...
                ty = (convert_direct(yaw,150,di))
                tp = (convert_direct(pitch,150,di))
                tr = (convert_direct(roll,100,dis))
            ...
            yerror, perror,rerror = measure_MAE(cont_labels,yaw,pitch,roll,gpu,convert_direct,di,dis)
        ...
        cur_MAE= ((ts1yaw_error+ts1pitch_error+ts1roll_error)/tstotal)
        if cur_MAE<=best_MAE:
                 print("best:")
                 best_MAE=cur_MAE
