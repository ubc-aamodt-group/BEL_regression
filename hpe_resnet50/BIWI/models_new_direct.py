...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,num_bits=10,code='u'):
        ...
        print(layers)
        code_bits={'u':[150,100],'j':[75,50],'b1jdj':[39,26],'b2jdj':[21,15],'hex16':[16,16],'rand16':[16,16]}
        self.inplanes = 64
    ...
    def forward(self, x):
        ...
        #yaw  = self.yawm(yaw)
        #yaw= (torch.mm(yaw,self.yawm))-self.yawb


        pitch = self.fc_angles_pitch(x)
        #pitch  = self.pitchm(pitch)
        #pitch= (torch.mm(pitch,self.pitchm))-self.pitchb

        roll = self.fc_angles_roll(x)
        #roll  = self.rollm(roll)
        #roll= (torch.mm(roll,self.rollm))-self.rollb
        
        #y=torch.mm(F.sigmoid(yaw),self.scales) * 150./64.
        #p=torch.mm(F.sigmoid(pitch),self.scales) * 150./64.
        #r=torch.mm(F.sigmoid(roll),self.scales) * 100./64.
