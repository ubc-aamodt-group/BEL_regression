...
def main():
	f=open("overall_filelist_clean_reference.txt","a")
	f2=open("overall_filelist.txt","a")
	f3=open("overall_filelist_removed.txt","a")
	args = get_args()
	mypath = args.db
	...
	for i in tqdm(range(len(onlyfiles_jpg))):
		...
		cv2.imwrite(mypath+'/'+img_name, img)
		cont_labels = np.array([yaw, pitch, roll])

		if (pitch >=-99 and pitch <= 99) and (yaw >=-99 and yaw <= 99) and (roll >=-99 and roll <= 99):
			f.write("%s\n"%(mypath+'/'+img_name))
		else:
			f3.write("%s\n"%(mypath+'/'+img_name))
		f2.write("%s\n"%(mypath+'/'+img_name))
